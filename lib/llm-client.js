...
function getRuntimeModelConfig() {
  ...
  const llmCompatBaseUrl = pickEnv("LLM_COMPAT_BASE_URL", pickEnv("OPENAI_COMPAT_BASE_URL", "https://openrouter.ai/api/v1"));
  ...
  return {
    ...
    openaiCompatBaseUrl: llmCompatBaseUrl,
  };
}
...
async function generateCompletion({ systemPrompt, userPrompt, runtime = {} }) {
  ...
  if (cfg.provider === "openai-compat") {
    const apiKey = runtime.llmCompatApiKey
      || runtime.openaiCompatApiKey
      || pickEnv("LLM_COMPAT_API_KEY", pickEnv("OPENAI_COMPAT_API_KEY"));
    if (!apiKey) {
      throw new Error("LLM_COMPAT_API_KEY not configured");
    }
